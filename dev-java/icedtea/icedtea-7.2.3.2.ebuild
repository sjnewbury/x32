...
java_prepare() {
	...
	epatch "${FILESDIR}"/${P}-no-x86-march.patch
	epatch "${FILESDIR}"/${P}-x32.patch
	epatch "${FILESDIR}"/${P}-smartcardio-append-cflags.patch

	export DISTRIBUTION_PATCHES="patches/no-x86-march.patch patches/x32.patch patches/smartcardio-append-cflags.patch"
...
src_configure() {
	...
	export OTHER_CFLAGS="${CFLAGS}" OTHER_LDFLAGS="${LDFLAGS}"
