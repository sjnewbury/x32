...
inherit eutils flag-o-matic python-single-r1 toolchain-funcs versionator ${GIT_ECLASS}
...
if [[ ${PV} == 9999* ]]; then
	EGIT_REPO_URI=https://github.com/boostorg/boost.git
	EGIT_BRANCH=develop
	SRC_URI=
else
...
src_prepare() {
	epatch \
		...
		"${FILESDIR}/${PN}-1.58.0-x32.patch" \
		"${FILESDIR}/${P}-no-setup-gcc-address-model.patch"
	...
	sed -i \
		-e '/def cpu_flags/,$ d' \
		-e '/local rule cpu-flags/,$ d' \
		tools/gcc.{jam,py} || die "sed failed"
