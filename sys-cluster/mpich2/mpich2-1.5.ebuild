...
inherit fortran-2 autotools multilib-minimal
...
COMMON_DEPEND="
	...
	sys-apps/hwloc[${MULTILIB_USEDEP}]
...
src_prepare() {
	...
	epatch "${FILESDIR}/${P}-cpi-flags.patch"
	epatch "${FILESDIR}/${P}-examples-AM_-prefix.patch"
...
multilib_src_install() {
	default
	GFORTRAN_MODULE_PATH=$(${FC} -print-file-name=finclude)
	einfo "Moving fortran modules into ${GFORTRAN_MODULE_PATH}"
	dodir "${GFORTRAN_MODULE_PATH}"
	mv "${ED}"/usr/include/*.mod "${ED}"/"${GFORTRAN_MODULE_PATH}" || die
}
