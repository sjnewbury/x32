...
src_prepare() {
	...
	epatch "${FILESDIR}/festival-linkflags.patch"

	# Apply speech-tools patches
	pushd "${WORKDIR}"/speech_tools
	epatch "${FILESDIR}/speech-tools-respect-more-flags.patch"
	popd
...
src_compile() {
	emake -j1 PROJECT_LIBDEPS="" REQUIRED_LIBDEPS="" LOCAL_LIBDEPS="" \
		...
		LDFLAGS="${LDFLAGS}" CC_OTHER_FLAGS="${CFLAGS}" \
		CXX_OTHER_FLAGS="${CXXFLAGS}" LINK_OTHER_FLAGS="${LDFLAGS}" \
